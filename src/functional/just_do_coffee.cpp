                  ...
                  const bool brewActive)
{
    ...
    { // if brewState == true
        ...
        else
        {
            ...
            if (runningCfg.brewDeltaState)
            {
                ...
                float BREW_TEMP_DELTA = mapRange(sensorTemperature, brewTempSetPoint,
                                                 brewTempSetPoint + tempDelta, tempDelta, 0, 0);
                deltaOffset = constrain(BREW_TEMP_DELTA, 0, tempDelta);
            }
            if (sensorTemperature <= brewTempSetPoint + deltaOffset)
            {
                pulseHeaters(runningCfg.hpwr, runningCfg.mainDivider, runningCfg.brewDivider,
                             brewActive);
            }
            else
            {
    ...
    { // if brewState == false
        ...
        else
        {
            ...
            int HPWR_OUT = mapRange(sensorTemperature, brewTempSetPoint - 10, brewTempSetPoint,
                                    runningCfg.hpwr, HPWR_LOW, 0);
            HPWR_OUT =
                constrain(HPWR_OUT, HPWR_LOW,
                          runningCfg.hpwr); // limits range of sensor values to HPWR_LOW and HPWR
    ...
    { // keep steam boiler supply valve open while steaming/descale only
        gpio::setSteamValveRelayOff();
    }
    gpio::setSteamBoilerRelayOff();
}

void pulseHeaters(const uint32_t pulseLength, const int factor_1, const int factor_2,
                  const bool brewActive)
{
    ...
    {
        brewActive ? gpio::setBoilerOff() : gpio::setBoilerOn();
        heaterState = !heaterState;
        heaterWave = millis();
    }
    else if (heaterState && ((millis() - heaterWave) > (pulseLength / factor_2)))
    {
        brewActive ? gpio::setBoilerOn() : gpio::setBoilerOff();
        heaterState = !heaterState;
        heaterWave = millis();
    }
}
...
void steamCtrl(const eepromValues_t &runningCfg, SensorState &currentState)
{
    currentState.steamSwitchState
        ...
        : lcdTargetState((int)HEATING::MODE_brew); // setting the steam/hot water target temp
