...
static void sensorReadSwitches(void) {
  ...
  currentState.hotWaterSwitchState =
      ...
      (currentState.brewSwitchState &&
       currentState.steamSwitchState); // use either an actual switch, or the
                                       // GC/GCP switch combo
}
    ...
    if (currentState.scalesPresent) {
      ...
      if (brewActive) {
        currentState.shotWeight =
            currentState.tarePending ? 0.f : currentState.weight;
...
static void sensorsReadPressure(void) {
  ...
  if (elapsedTime > GET_PRESSURE_READ_EVERY) {
    ...
    currentState.pressureChangeSpeed =
        (currentState.smoothedPressure - previousSmoothedPressure) /
        elapsedTimeSec;
...
static long sensorsReadFlow(float elapsedTimeSec) {
  ...
  currentState.pumpFlow =
      getPumpFlow(currentState.pumpClicks, currentState.smoothedPressure);
  ...
  currentState.pumpFlowChangeSpeed =
      (currentState.smoothedPumpFlow - previousSmoothedPumpFlow) /
      elapsedTimeSec;
  return pumpClicks;
}
...
static void calculateWeightAndFlow(void) {
  ...
  if (brewActive) {
    // Marking for tare in case smth has gone wrong and it has exited tare
    // already.
    ...
    if (elapsedTime > REFRESH_FLOW_EVERY) {
      ...
      if (predictiveWeight.isOutputFlow() || currentState.weight > 0.4f) {
        ...
        /* Probabilistically the flow is lower if the shot is just started
        winding up and we're flow profiling, once pressure stabilises around the
        setpoint the flow is either stable or puck restriction is high af. */
        ...
        currentState.consideredFlow =
            smoothConsideredFlow.updateEstimate(actualFlow);
...
static void pageValuesRefresh() {
  ...
  // Or maybe it's a page that needs constant polling
    ...
    lcdFetchPage(runningCfg, lcdCurrentPageId, runningCfg.activeProfile);
  // Finally read the page we left, as it could've been changed in place (e.g.
  // boolean toggles)
  else
 ...
 * pressure read for the LCD alone*/
...
#else
    lcdSetPressure(currentState.pressure > 0.f ? currentState.pressure * 10.f
                                               : 0.f);
#endif

    /*LCD temp output*/
    float brewTempSetPoint =
        ACTIVE_PROFILE(runningCfg).setpoint + runningCfg.offsetTemp;
    // float liveTempWithOffset = currentState.temperature -
    // runningCfg.offsetTemp;
    currentState.waterTemperature =
        (currentState.temperature >
             (float)ACTIVE_PROFILE(runningCfg).setpoint &&
         currentState.brewSwitchState)
            ? currentState.temperature / (float)brewTempSetPoint +
                  (float)ACTIVE_PROFILE(runningCfg).setpoint
            : currentState.temperature;
    ...
    case NextionPage::Home:
      ...
      tempDecimal = (currentState.waterTemperature -
    ...
    case NextionPage::BrewManual:
      ...
      tempDecimal = (currentState.waterTemperature -
                    ...
                    10;
      lcdSetTemperatureDecimal(tempDecimal);
      // If the weight output is a negative value lower than -0.8 you might want
      // to tare again before extraction starts.
      if (currentState.shotWeight)
        lcdSetWeight(currentState.shotWeight > -0.8f ? currentState.shotWeight
                                                     : -0.9f);
      /*LCD flow output*/
    ...
    if (brewActive) {
      lcdSetBrewTimer((millis() > brewingTimer)
                          ? (int)((millis() - brewingTimer) / 1000)
                          : 0);
      lcdBrewTimerStart();  // nextion timer start
      lcdWarmupStateStop(); // Flagging warmup notification on Nextion needs to
                            // stop (if enabled)
    } else {
...
lcdSwitchActiveToStoredProfile(const eepromValues_t &storedSettings) {
  ...
  ACTIVE_PROFILE(runningCfg) =
      storedSettings.profiles[runningCfg.activeProfile];
  updateProfilerPhases();
...
void lcdResetSettingsTrigger(void) { tryEepromWrite(eepromGetDefaultValues()); }
...
void lcdRefreshElementsTrigger(void) {
  ...
  case NextionPage::BrewPreinfusion:
    ACTIVE_PROFILE(eepromCurrentValues).preinfusionFlowState =
        lcdGetPreinfusionFlowState();
    break;
  case NextionPage::BrewProfiling:
    ACTIVE_PROFILE(eepromCurrentValues).mfProfileState =
        lcdGetProfileFlowState();
    break;
...
static void updateProfilerPhases(void) {
  ...
  if (ACTIVE_PROFILE(runningCfg).stopOnWeightState) {
    shotTarget = (ACTIVE_PROFILE(runningCfg).shotStopOnCustomWeight < 1.f)
                     ? ACTIVE_PROFILE(runningCfg).shotDose *
                           ACTIVE_PROFILE(runningCfg).shotPreset
                     : ACTIVE_PROFILE(runningCfg).shotStopOnCustomWeight;
  }

  // update global stop conditions (currently only stopOnWeight is configured in
  // nextion)
...
static void addPreinfusionPhases() {
  if (ACTIVE_PROFILE(runningCfg)
          .preinfusionFlowState) { // flow based PI enabled
    float isPressureAbove =
        ACTIVE_PROFILE(runningCfg).preinfusionPressureAbove
            ? ACTIVE_PROFILE(runningCfg).preinfusionFlowPressureTarget
            : -1.f;
    float isWeightAbove =
        ACTIVE_PROFILE(runningCfg).preinfusionWeightAbove > 0.f
            ? ACTIVE_PROFILE(runningCfg).preinfusionWeightAbove
            : -1.f;
    float isWaterPumped = ACTIVE_PROFILE(runningCfg).preinfusionFilled > 0.f
                              ? ACTIVE_PROFILE(runningCfg).preinfusionFilled
                              : -1.f;

    addFlowPhase(Transition{ACTIVE_PROFILE(runningCfg).preinfusionFlowVol},
                 ACTIVE_PROFILE(runningCfg).preinfusionFlowPressureTarget,
                 ACTIVE_PROFILE(runningCfg).preinfusionFlowTime * 1000,
                 isPressureAbove, -1, isWeightAbove, isWaterPumped);
  } else { // pressure based PI enabled
    // For now handling phase switching on restrictions here but as this grow
    // will have to deal with it otherwise.
    float isPressureAbove = ACTIVE_PROFILE(runningCfg).preinfusionPressureAbove
                                ? ACTIVE_PROFILE(runningCfg).preinfusionBar
                                : -1.f;
    float isWeightAbove =
        ACTIVE_PROFILE(runningCfg).preinfusionWeightAbove > 0.f
            ? ACTIVE_PROFILE(runningCfg).preinfusionWeightAbove
    ...
    addPressurePhase(Transition{ACTIVE_PROFILE(runningCfg).preinfusionBar},
                     ACTIVE_PROFILE(runningCfg).preinfusionPressureFlowTarget,
                     ACTIVE_PROFILE(runningCfg).preinfusionSec * 1000,
                     isPressureAbove, -1, isWeightAbove, isWaterPumped);
  }
}

static void addSoakPhase() {
  uint16_t phaseSoak = ACTIVE_PROFILE(runningCfg).preinfusionFlowState
                           ? ACTIVE_PROFILE(runningCfg).soakTimeFlow
                           : ACTIVE_PROFILE(runningCfg).soakTimePressure;
  float maintainFlow = ACTIVE_PROFILE(runningCfg).soakKeepFlow > 0.f
                           ? ACTIVE_PROFILE(runningCfg).soakKeepFlow
                           : -1.f;
  float maintainPressure = ACTIVE_PROFILE(runningCfg).soakKeepPressure > 0.f
                               ? ACTIVE_PROFILE(runningCfg).soakKeepPressure
                               : -1.f;
  float isPressureBelow = ACTIVE_PROFILE(runningCfg).soakBelowPressure > 0.f
                              ? ACTIVE_PROFILE(runningCfg).soakBelowPressure
                              : -1.f;
  float isPressureAbove = ACTIVE_PROFILE(runningCfg).soakAbovePressure > 0.f
                              ? ACTIVE_PROFILE(runningCfg).soakAbovePressure
                              : -1.f;
  float isWeightAbove = ACTIVE_PROFILE(runningCfg).soakAboveWeight > 0.f
                            ? ACTIVE_PROFILE(runningCfg).soakAboveWeight
                            : -1.f;

  if (maintainPressure > 0.f)
    addPressurePhase(Transition{maintainPressure},
                     (maintainFlow > 0.f ? maintainFlow : 2.5f),
                     phaseSoak * 1000, isPressureAbove, isPressureBelow,
                     isWeightAbove, -1);
  else if (maintainFlow > 0.f)
    addFlowPhase(Transition{maintainFlow}, -1, phaseSoak * 1000,
                 isPressureAbove, isPressureBelow, isWeightAbove, -1);
  else
    addPressurePhase(Transition{maintainPressure}, maintainFlow,
                     phaseSoak * 1000, isPressureAbove, isPressureBelow,
                     isWeightAbove, -1);
}
  ...
  if (ACTIVE_PROFILE(runningCfg).profilingState) {
    if (ACTIVE_PROFILE(runningCfg).tpState) {
      ...
      if (ACTIVE_PROFILE(runningCfg).tpType) { // flow based profiling enabled
        ...
        float holdLimit = ACTIVE_PROFILE(runningCfg).tfProfileHoldLimit > 0.f
                              ? ACTIVE_PROFILE(runningCfg).tfProfileHoldLimit
                              : -1;
        TransitionCurve curve =
            (TransitionCurve)ACTIVE_PROFILE(runningCfg).tfProfileSlopeShape;
        ...
        if (fpStart > 0.f && fpHold > 0) {
          ...
          rampPhaseIndex =
              rampPhaseIndex > 0 ? rampPhaseIndex : profile.phaseCount() - 1;
        }
        addFlowPhase(Transition{fpStart, fpEnd, curve, curveTime},
                     ACTIVE_PROFILE(runningCfg).tfProfilingPressureRestriction,
                     curveTime, -1, -1, -1, -1);
        rampPhaseIndex =
            rampPhaseIndex > 0 ? rampPhaseIndex : profile.phaseCount() - 1;
        ...
        float holdLimit = ACTIVE_PROFILE(runningCfg).tpProfilingHoldLimit > 0.f
                              ? ACTIVE_PROFILE(runningCfg).tpProfilingHoldLimit
                              : -1;
        TransitionCurve curve =
            (TransitionCurve)ACTIVE_PROFILE(runningCfg).tpProfilingSlopeShape;
        uint16_t curveTime = ACTIVE_PROFILE(runningCfg).tpProfilingSlope * 1000;
        /* ------------------------------------------ */

        if (ppStart > 0.f && ppHold > 0) {
          addPressurePhase(Transition{ppStart}, holdLimit, ppHold, -1, -1, -1,
                           -1);
          rampPhaseIndex =
              rampPhaseIndex > 0 ? rampPhaseIndex : profile.phaseCount() - 1;
        }
        addPressurePhase(Transition{ppStart, ppEnd, curve, curveTime},
                         ACTIVE_PROFILE(runningCfg).tpProfilingFlowRestriction,
                         curveTime, -1, -1, -1, -1);
        rampPhaseIndex =
            rampPhaseIndex > 0 ? rampPhaseIndex : profile.phaseCount() - 1;
      }
    ...
    if (ACTIVE_PROFILE(runningCfg)
      ...
      TransitionCurve curve =
          (TransitionCurve)ACTIVE_PROFILE(runningCfg).mpProfilingSlopeShape;
      uint16_t curveTime = ACTIVE_PROFILE(runningCfg).mpProfilingSlope * 1000;
      /* ------------------------------------------ */
      addPressurePhase(Transition(ppStart, ppEnd, curve, curveTime),
                       ACTIVE_PROFILE(runningCfg).mpProfilingFlowRestriction,
                       -1, -1, -1, -1, -1);
    }
...
static void insertRampPhaseIfNeeded(size_t rampPhaseIndex) {
  ...
  TransitionCurve rampCurve =
      (TransitionCurve)ACTIVE_PROFILE(runningCfg).preinfusionRampSlope;

  if (rampPhaseIndex <= 0 || rampTime <= 0 ||
      rampCurve == TransitionCurve::INSTANT) { // No ramp needed
    return;
  }

  // Get the phase currently in rampPhaseIndex - this is the phase we want to
  // ramp to
  Phase targetPhase = profile.phases[rampPhaseIndex];
  float targetValue = targetPhase.target.isInstant() ? targetPhase.target.end
                                                     : targetPhase.target.start;
...
static void addPressurePhase(Transition pressure, float flowRestriction,
                             ...
                             float isWaterPumped) {
  addPhase(PHASE_TYPE::PHASE_TYPE_PRESSURE, pressure, flowRestriction, timeMs,
           pressureAbove, pressureBelow, shotWeight, isWaterPumped);
}
...
static void addPhase(PHASE_TYPE type, Transition target, float restriction,
                     ...
                     float shotWeight, float isWaterPumped) {
  profile.addPhase(Phase{.type = type,
                         .target = target,
                         ...
                         .stopConditions = PhaseStopConditions{
                             ...
                             .weight = shotWeight,
                             .waterPumpedInPhase = isWaterPumped}});
}

void onProfileReceived(Profile &newProfile) {}

static void profiling(void) {
  if (brewActive) { // runs this only when brew button activated and pressure
                    // profile selected
    ...
    ShotSnapshot shotSnapshot =
        buildShotSnapshot(timeInShot, currentState, currentPhase);
...
static void brewDetect(void) {
  ...
  if (currentState.brewSwitchState) {
    ...
    // needs to be here as it creates a locking state soemtimes if not kept up
    // to date during brew mainly when shotWeight restriction kick in.
...
static inline void sysHealthCheck(float pressureThreshold) {
  ...
  /* This *while* is here to prevent situations where the system failed to get a
  temp reading and temp reads as 0 or -7(cause of the offset) If we would use a
  non blocking function then the system would keep the SSR in HIGH mode which
  would most definitely cause boiler overheating */
  while (currentState.temperature <= 0.0f || currentState.temperature == NAN ||
    ...
    we force set it to LOW while trying to get a temp reading - IMPORTANT safety
    feature */
    setPumpOff();
    setBoilerOff();
    setSteamBoilerRelayOff();
    if (millis() > thermoTimer) {
      ...
      currentState.temperature =
          thermocoupleRead() -
          runningCfg.offsetTemp; // Making sure we're getting a value
      thermoTimer = millis() + GET_KTYPE_READ_EVERY;
    }
  }
...
// Releasing the excess pressure after steaming or brewing if necessary
  ...
  if (currentState.smoothedPressure >= pressureThreshold &&
      currentState.temperature < 100.f) {
    if (millis() >= systemHealthTimer - 3500ul &&
        millis() <= systemHealthTimer - 500ul) {
      char tmp[25];
...
static bool isBoilerFillPhase(unsigned long elapsedTime) {
  return lcdCurrentPageId == NextionPage::Home &&
         elapsedTime >= BOILER_FILL_START_TIME;
}
...
static bool isBoilerFull(unsigned long elapsedTime) {
  ...
  if (elapsedTime > BOILER_FILL_START_TIME + 1000UL) {
    boilerFull =
        (previousSmoothedPressure - currentState.smoothedPressure > -0.02f) &&
