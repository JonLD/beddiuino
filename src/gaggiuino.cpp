...
static void lcdSwitchActiveToStoredProfile(const eepromValues_t &storedSettings);
static bool isBoilerFillPhase(unsigned long elapsedTime);
static void addFillBasketPhase(float flowRate);
static void addPressurePhase(Transition pressure, float flowRestriction, int timeMs,
                             float pressureAbove, float pressureBelow, float shotWeight,
                             float isWaterPumped);
static void addFlowPhase(Transition flow, float pressureRestriction, int timeMs,
                         float pressureAbove, float pressureBelow, float shotWeight,
                         float isWaterPumped);
static void addPhase(PHASE_TYPE type, Transition target, float restriction, int timeMs,
                     float pressureAbove, float pressureBelow, float shotWeight,
                     float isWaterPumped);
...
void setup(void)
{
    ...
    gpio::setBoilerOff(); // relayPin LOW
...
static void modeSelect(void)
{
    ...
    case OPERATION_MODES::OPMODE_flush:
        ...
        brewActive ? gpio::setBoilerOff() : justDoCoffee(runningCfg, currentState, false);
        break;
 ...
 * pressure read for the LCD alone*/
        ...
        currentState.waterTemperature =
            (currentState.temperature > (float)ACTIVE_PROFILE(runningCfg).setpoint &&
        ...
        case NextionPage::Home:
            ...
            tempDecimal =
                (currentState.waterTemperature - (uint16_t)currentState.waterTemperature) * 10;
        ...
        if (brewActive)
        {
            lcdSetBrewTimer((millis() > brewingTimer) ? (int)((millis() - brewingTimer) / 1000)
                                                      : 0);
...
static void updateProfilerPhases(void)
{
    ...
    {
        shotTarget =
            (ACTIVE_PROFILE(runningCfg).shotStopOnCustomWeight < 1.f)
...
static void addPreinfusionPhases()
{
    ...
    { // flow based PI enabled
        float isPressureAbove = ACTIVE_PROFILE(runningCfg).preinfusionPressureAbove
                                    ...
                                    : -1.f;
        float isWeightAbove = ACTIVE_PROFILE(runningCfg).preinfusionWeightAbove > 0.f
        ...
        addFlowPhase(Transition{ACTIVE_PROFILE(runningCfg).preinfusionFlowVol},
                     ACTIVE_PROFILE(runningCfg).preinfusionFlowPressureTarget,
                     ACTIVE_PROFILE(runningCfg).preinfusionFlowTime * 1000, isPressureAbove, -1,
                     isWeightAbove, isWaterPumped);
    }
    else
    { // pressure based PI enabled
        ...
        addPressurePhase(Transition{ACTIVE_PROFILE(runningCfg).preinfusionBar},
                         ...
                         ACTIVE_PROFILE(runningCfg).preinfusionSec * 1000, isPressureAbove, -1,
                         isWeightAbove, isWaterPumped);
    }
}
...
static void addSoakPhase()
{
    ...
    if (maintainPressure > 0.f)
        addPressurePhase(Transition{maintainPressure}, (maintainFlow > 0.f ? maintainFlow : 2.5f),
                         phaseSoak * 1000, isPressureAbove, isPressureBelow, isWeightAbove, -1);
    else if (maintainFlow > 0.f)
        addFlowPhase(Transition{maintainFlow}, -1, phaseSoak * 1000, isPressureAbove,
                     isPressureBelow, isWeightAbove, -1);
    else
        addPressurePhase(Transition{maintainPressure}, maintainFlow, phaseSoak * 1000,
...
static void addMainExtractionPhasesAndRamp()
{
    ...
    if (ACTIVE_PROFILE(runningCfg).profilingState)
    {
        if (ACTIVE_PROFILE(runningCfg).tpState)
        {
            ...
            { // flow based profiling enabled
                ...
                addFlowPhase(Transition{fpStart, fpEnd, curve, curveTime},
                             ACTIVE_PROFILE(runningCfg).tfProfilingPressureRestriction, curveTime,
                             -1, -1, -1, -1);
                rampPhaseIndex = rampPhaseIndex > 0 ? rampPhaseIndex : profile.phaseCount() - 1;
            }
            else
            { // pressure based profiling enabled
                ...
                addPressurePhase(Transition{ppStart, ppEnd, curve, curveTime},
                                 ACTIVE_PROFILE(runningCfg).tpProfilingFlowRestriction, curveTime,
                                 -1, -1, -1, -1);
                rampPhaseIndex = rampPhaseIndex > 0 ? rampPhaseIndex : profile.phaseCount() - 1;
            }
        }
        ...
        { // flow based profiling enabled
            ...
            /* ------------------------------------------ */
            addFlowPhase(Transition(fpStart, fpEnd, curve, curveTime),
                         ACTIVE_PROFILE(runningCfg).mfProfilingPressureRestriction, -1, -1, -1, -1,
                         -1);
        }
        else
        { // pressure based profiling enabled
            ...
            /* ------------------------------------------ */
            addPressurePhase(Transition(ppStart, ppEnd, curve, curveTime),
                             ACTIVE_PROFILE(runningCfg).mpProfilingFlowRestriction, -1, -1, -1, -1,
                             -1);
        }
    }
    else
...
static void insertRampPhaseIfNeeded(size_t rampPhaseIndex)
{
    ...
    float targetValue =
        targetPhase.target.isInstant() ? targetPhase.target.end : targetPhase.target.start;
    ...
    profile.insertPhase(Phase{.type = targetPhase.type,
                             ...
                             float isWaterPumped)
{
    addPhase(PHASE_TYPE::PHASE_TYPE_PRESSURE, pressure, flowRestriction, timeMs, pressureAbove,
             pressureBelow, shotWeight, isWaterPumped);
}
                     ...
                     float isWaterPumped)
{
    profile.addPhase(
        Phase{.type = type,
              ...
              .stopConditions = PhaseStopConditions{.time = timeMs,
